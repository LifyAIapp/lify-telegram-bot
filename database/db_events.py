...
async def add_event(owner_user_id: str, title: str, description: str, date: str, is_shared: bool = False):
...
async def update_event(event_id: int, title: str, description: str, date: str, is_shared: bool):
    conn = await get_connection()
    try:
        await conn.execute(
            """
...
async def get_upcoming_events(days_ahead: int = 3):
    conn = await get_connection()
    try:
        rows = await conn.fetch(
            """
            SELECT * FROM events
            WHERE date BETWEEN CURRENT_DATE AND CURRENT_DATE + $1
            ORDER BY date
            """,
            days_ahead
        )
