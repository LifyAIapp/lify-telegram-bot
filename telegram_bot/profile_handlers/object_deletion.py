...
# 🔍 Показать список объектов для удаления
async def show_objects_for_deletion(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not isinstance(section_id, int):
        await update.message.reply_text("⚠️ Не удалось определить корректный раздел или подраздел.")
        return
    ...
    # Сохраняем имена и ID объектов-кандидатов на удаление
